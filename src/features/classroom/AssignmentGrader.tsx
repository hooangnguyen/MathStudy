...
export const AssignmentGrader: React.FC<AssignmentGraderProps> = ({ onClose, assignmentTitle, className, classId, assignmentId }) => {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-[100] bg-slate-900/20 backdrop-blur-sm flex justify-center"
      onClick={onClose}
    >
      <motion.div
        initial={{ x: '100%' }}
        ...
        className="w-full h-full bg-slate-50 flex flex-col"
                      ...
                      <div key={answer.questionId} className="bg-white p-4 md:p-6 rounded-[2rem] border border-slate-200 shadow-sm space-y-4">
                              ...
                              <div className="space-y-1">
                                {answer.answer.map((idx: number) => (
                                  <div key={idx} className="flex items-center gap-2">
                                    ...
                                    <MathRenderer content={answer.options?.[idx] || `Tùy chọn ${idx + 1}`} />
                                  </div>
                                ))}
                                {answer.answer.length === 0 && <span className="text-slate-400">(Không chọn đáp án nào)</span>}
                              </div>
                            ) : answer.type === 'multiple_choice' ? (
                              <span><MathRenderer content={answer.options?.[answer.answer] || `Tùy chọn ${Number(answer.answer) + 1}`} /></span>
                            ) : (
                              <span><MathRenderer content={answer.answer || ''} /></span>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="p-4 bg-emerald-50 rounded-2xl border border-emerald-100 space-y-2">
                          ...
                          <div className="font-medium text-emerald-800 whitespace-pre-wrap">
                            {answer.type === 'multiple_choice' ? (
                              <span><MathRenderer content={answer.options?.[answer.correctAnswer] || `Tùy chọn ${Number(answer.correctAnswer) + 1}`} /></span>
